...
def filter_per_film(data, min_synopsis_words, max_non_latin_chars, max_emojis):
    ...
    for r in reviews:
        ...
        if (count_emojis(review_text) <= max_emojis and
            count_non_latin_chars(review_text) <= max_non_latin_chars):
...
def filter_per_review(data, min_synopsis_words, max_non_latin_chars, max_emojis): # each review has its own entry with its film metadata
    if min_synopsis_words > 0 and not has_sufficient_synopsis(data.get("synopsis"), min_synopsis_words):
        return None
...
def main():
    ...
    parser.add_argument("--max-emojis", type=int, default=DEFAULT_MAX_EMOJIS,
            help=f"Maximum number of emojis allowed per review (default: {DEFAULT_MAX_EMOJIS})")
    parser.add_argument("--max-non-latin-chars", type=int, default=DEFAULT_MAX_NON_LATIN_CHARS,
        ...
        for index, line in enumerate(infile, 1):
            try:
                ...
                if args.mode == "film":
                    ...
                    review_entries = filter_per_review(data, args.min_synopsis_words, args.max_non_latin_chars, args.max_emojis)
                    if review_entries:
