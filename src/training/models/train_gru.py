...
def main(config: Config):
    ...
    assert isinstance(config.training, GRUTrainingConfig)
    ...
    checkpoint = {
        str(GruCheckpointEnum.MODEL): model.state_dict(),
        str(GruCheckpointEnum.VOCAB_SIZE): vocab_size,
        str(GruCheckpointEnum.NUM_PARAMS): num_params,
        str(GruCheckpointEnum.CONFIG): asdict(config),
        str(GruCheckpointEnum.TOKENIZER): tokenizer,
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train GRU Language Model")
    ...
    if config.model.type != ModelTypeEnum.GRU:
        raise ValueError(f"Config '{args.config}' is not a GRU config")
